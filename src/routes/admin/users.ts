 ...
 */
router.get('/search', requireSuperAdmin(), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.post('/tenants/:tenantId/:userId/roles', requireSuperAdmin(), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (role.scope !== 'tenant') {
      return res.status(400).json({ error: 'Only tenant-scoped roles can be assigned to tenants' });
    }
 ...
 */
router.delete('/tenants/:tenantId/:userId/roles/:roleId', requireSuperAdmin(), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/tenants/:tenantId/:userId/roles', requireSuperAdmin(), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { tenantId, userId } = req.params;
